...
def end_to_base():
        ...
        phi, theta, psi, x, y, z = 2.8646582694027454,-0.25028606900669015,-0.25028606900669015,-0.25028606900669015,-0.25028606900669015,-0.25028606900669015
        ...
        # print(pose.shape)
        # print(angle.shape)
        RT_=pose_robot(angle[0, 0],angle[0, 1],angle[0, 2],pose[0, 0],pose[0, 1],pose[0, 2])
...
def cam_to_world(external_image_path, chess_board_x_num, chess_board_y_num, K, chess_board_len):
    R_cam_to_base = calculate_external(external_image_path, chess_board_x_num, chess_board_y_num, K, chess_board_len)
    R_base_to_world = np.array([[1, 0, 0, 0],
                                [0, 1, 0, 1], # according the actual position
                                [0, 0, 1, 0],
                                [0, 0, 0, 1]])
    R_cam_to_world = R_base_to_world @ R_cam_to_base
    return R_base_to_world
...
def save_image(): 
    ...
    save_image_path = '/home/zhangyuxin/furniture-bench-main/save_chessboard/test_camera_external'
...
#     def __init__(self):
#         self.num = 28  #用于标定的图片数，需要根基实际保存的图片数进行修改
